...
FILTERING_INITIAL_MAPPING1(){
	...
	tmp="$SAMTOOLS sort $INPUT_DIR/$SAMPLE_NAME.pgf/pgf.accepted_hits.bam $DIR_FILTER/$SAMPLE_NAME.sorted"
	echo $tmp
...
FILTERING_INITIAL_MAPPING2(){


	for hap in cox apd dbb mann mcf qbl ssto;
	do 
	
		DIR_FILTER="$OUTPUT_DIR/$SAMPLE_NAME.$hap"
		if [[ ! -e $DIR_FILTER ]]; then
			echo "mkdir -p $DIR_FILTER"
		fi
		...
		tmp="$SAMTOOLS sort $INPUT_DIR/$SAMPLE_NAME.$hap/accepted_hits.bam $DIR_FILTER/$SAMPLE_NAME.sorted"
		echo $tmp
		tmp="$SAMTOOLS index $DIR_FILTER/$SAMPLE_NAME.sorted.bam"
		echo $tmp
...
# /well/jknight/age_cohort_monocytes/2.mapping/MET1mM_632.pgf/test
...
WRITE_A_SCRIPT(){
        ...
        ENVIRONMENT_SETUP
	FILTERING_INITIAL_MAPPING1 $1
        FILTERING_INITIAL_MAPPING2 $1
