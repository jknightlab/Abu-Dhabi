...
This comprises samples from 19 individuals (9 controls, 10 with type 2 diabetes -
patient 115 updated to T2D based on full clinical information although listed
as a control).
...
Clinical information looked up from /groups/jknight/Group_members_personal_data_storage/Lawrence/Abu Dabi Patient medical data/Collated Family information.xlsm
...
The mapping quality control metrics are explored:
...
ggplot(metrics, aes(Total.Purity.Filtered.Reads.Sequenced, Mapped,
  ...
  ggtitle("Mapped Reads vs Total Reads Sequenced", 
          "This highlights four samples (4 x R115) where a large number of reads were sequenced but not mapped") +
  ylab("Number of reads mapped to human genome") +
...
ggplot(metrics, aes(SampleType, Mapping.Rate)) +
  ...
  ggtitle("Mapping Rate",
          "Highlighting samples with very low mapping rates - 4 samples from R115, R86 CD14_LPS, 119 CD14_MET)") +
...
ggplot(metrics, aes(SampleType, Base.Mismatch.Rate)) +
  ...
  geom_point(aes(shape=Repeat, colour=Disease)) +
  theme_bw() +
  ggtitle("Base Mismatch Rate", "Low across all samples") +
...
Based on these plots, I decided to filter primarily by the mapping rate, and
remove the samples with less than 40% of reads mapped. These are:
...
Explanatory text and the protocol used are largely from
...
keep only rows that have at least 10 reads total (across all samples). Note that
more strict filtering is applied to increase power via independent filtering on
the mean of normalized counts within the results function.
...
Initially we want to look at an overview of the data and how the samples relate
to each other. Principal component analysis (PCA) takes all the gene expression 
data and converts it to a smaller number of independent variables (components)
that summarise the variation in the data.
...
RNA-seq counts, however, the expected variance grows with the mean. If one
performs PCA directly on a matrix of normalized counts, the resulting plot
...
to take the logarithm of the normalized count values plus a pseudocount of 1;
...
between diabetes and control samples. Repeat samples were thought to be of poorer
quality, but this does not seem to have an impact on the clustering here.
...
separation between LPS treated and non-LPS treated cells. However, there are a few
samples that don't fall within treatment clusters, and this seems to relate to the
patient number e.g. Patient 102 samples are swapped round, (before being removed
for the above QC reasons 115 all clustered together in the centre) and 148 are all
at the top left. 
...
and 104 look like the LPS treated samples were swapped with the non-treated
samples.
...
# 
# # density plot
# plot(density(log(dds.norm[, 1]+1)), main="Distribution of normalised intensities",
#      col=sf$SampleType[1], ylim=c(0, 0.7),
#      cex.main=0.8)
# for (i in 2:ncol(dds.norm)){
#   lines(density(log(dds.norm[, i]+1)), col=sf$SampleType[i])
# }
# legend("topright", legend=unique(sf$SampleType), 
#        col=c(1:8), lty=1, cex=0.5)
```
...
(independent filtering). These genes are removed from the results table.
...
keep <- rowSums(counts(lps.dds)) >= 10
lps.dds <- lps.dds[keep,]
...
# density plot
# plot(density(log(lps.dds.norm[, 1]+1)), 
#      main="Distribution of normalised intensities",
#      col=lps.dds$SampleType[1], ylim=c(0, 0.7),
#      cex.main=0.8)
# for (i in 2:ncol(lps.dds.norm)){
#   lines(density(log(lps.dds.norm[, i]+1)), col=lps.dds$SampleType[i])
# }
# legend("topright", legend=unique(lps.dds$SampleType), 
#        col=c(1:8), lty=1, cex=0.5)
...
across groups. There is 1 significant gene detected; however, the plot shows
that this is driven by expression in just 2 samples:
...
lps.dis.res$Gene <- mcols(lps.dds.dis)$Gene
...
for(i in 1:nrow(lps.dis.res[lps.dis.res$padj < 0.05, ])){
  gene <- rownames(lps.dis.res[lps.dis.res$padj < 0.05, ])[i]
  d <- plotCounts(lps.dds.dis, gene=gene, 
                intgroup=c("SampleType", "Disease", "PatientNumber"),
                returnData=TRUE)
  d <- subset(d, SampleType %in% c("CD14_LPS", "CD14_PBS"))
  gene.name <- as.character(mcols(lps.dds)$Gene[which(rownames(lps.dds) == gene)])
  if(gene.name == "") gene.name <- as.character(mcols(lps.dds)$gene_id[which(rownames(lps.dds) == gene)])
  plot.list[[i]] <- ggplot(d, aes(x=SampleType, y=count, color=Disease,
                                     group=PatientNumber)) + 
...
print("One dodgy-looking sample")
...
# results
pbs.met.res <- results(lps.dds, parallel=TRUE,
                       contrast=c("SampleType", "CD14_PBS", "CD14_MET"),
                   independentFiltering=TRUE, alpha=0.05)
pbs.met.res$Gene <- mcols(lps.dds)$Gene
...
d <- plotCounts(lps.dds, gene="ENSG00000242770", 
                intgroup=c("SampleType", "Disease", "PatientNumber"),
                returnData=TRUE)
d <- subset(d, SampleType %in% c("CD14_MET", "CD14_PBS"))
gene.name <- "RP11-180K7.1"
...
lps.met.res <- results(lps.dds, parallel=TRUE, 
                   contrast=c("SampleType", "CD14_LPS", "CD14_MET_LPS"),
                   independentFiltering=TRUE, alpha=0.05)
lps.met.res$Gene <- mcols(lps.dds)$Gene
...
# design
disease.dds <- lps.dds
# 135 samples remaining (19 individuals, 4-8 samples each)
...
g <- which(mcols(disease.dds)$gene_name == "SIRT1")
...
g <- which(mcols(disease.dds)$gene_name == "ZNF683")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "C3AR1")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "MUC20")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "SEPT8")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "RAB15")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$Gene == "TPRG1")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$Gene == "ITGB1")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$Gene == "ASNS")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$Gene == "KIAA1671")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
This analysis is repeated accounting for BMI in the model.


```{r disease-bmi, message=FALSE}
# design
disease.dds <- lps.dds
# 135 samples remaining (19 individuals, 4-8 samples each)
design(disease.dds) <- ~ BMI + Disease
table(disease.dds$SampleType)
...
g <- which(mcols(disease.dds)$gene_name == "SIRT1")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease", "BMI"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=BMI)) + 
  geom_boxplot() +
  geom_point(size=3) + 
  scale_y_log10() +
  theme_bw() +
...
The single gene that is strongly significant in CD14_MET is shown. It encodes a 
...
MT.res <- results(lps.dds, parallel=TRUE, independentFiltering=TRUE, alpha=0.05,
                  contrast=c("SampleType", "CD14", "CD8"))
MT.res$Gene <- mcols(lps.dds)$Gene
MT.res.LFC <- lfcShrink(lps.dds, 
                        contrast=c("SampleType", "CD14", "CD8"), parallel=TRUE)
...
pp.res <- results(lps.dds, parallel=TRUE, independentFiltering=TRUE, alpha=0.05,
                  contrast=c("SampleType", "PBMC", "Paxgene"))
pp.res$Gene <- mcols(lps.dds)$Gene
