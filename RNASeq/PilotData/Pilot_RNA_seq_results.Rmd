...
featureData <- data.frame(Ensembl=rownames(counts), 
                          ...
                          GeneType=genes$gene_biotype[match(rownames(counts),
                                                       genes$ensembl_gene_id)])
...
for(j in 1:10){
  ...
  de.plot.list[[j]] <- ggplot(d, aes(x=SampleType, y=count, color=Disease,
    ...
    geom_line() +
    ylab(gene.name) +
    geom_text(data=d[d$PatientNumber %in% c(94, 100, 102, 148), ],
            aes(label=PatientNumber))
}
...
ggplot(data.frame(lpsresOrdered), aes(log2FoldChange, -log10(padj))) +
  ...
  theme_bw() +
  geom_text_repel(data=subset(data.frame(lpsresOrdered), padj<5e-20),
                  aes(label=Gene))
```
...
d <- plotCounts(lps.dds.dis, gene="ENSG00000182217", 
                intgroup=c("SampleType", "Disease", "PatientNumber"),
                returnData=TRUE)
d <- subset(d, SampleType %in% c("CD14_LPS", "CD14_PBS"))
gene.name <- as.character(mcols(lps.dds)$Gene[which(rownames(lps.dds) == "ENSG00000182217")])
...
The single gene that is significant in CD14 is shown. It encodes a 
transcriptional repressor involved in the circadian clock. 

```{r repeat-genes-cd14, warning=FALSE}
g <- which(mcols(repeat.dds)$Gene == "PER1")
...
The single gene that is significant in CD14_MET is shown. It encodes a 
...
Finally, the gene that is significant in Paxgene is XIST, which is involved in 
X chromosome inactivation

```{r repeat-genes-paxgene, warning=FALSE}
g <- which(mcols(repeat.dds)$Gene == "XIST")
d <- plotCounts(repeat.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease", "PatientNumber", 
                           "Repeat.Sample"),
                returnData=TRUE)
