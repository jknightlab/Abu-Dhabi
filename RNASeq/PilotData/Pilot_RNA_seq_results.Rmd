...
for(i in 1:nrow(lps.dis.res[lps.dis.res$padj < 0.05, ])){
  ...
  d <- plotCounts(lps.dds.dis, gene=gene, 
                ...
                returnData=TRUE)
  d <- subset(d, SampleType %in% c("CD14_LPS", "CD14_PBS"))
  gene.name <- as.character(mcols(lps.dds)$gene_name[which(rownames(lps.dds) == gene)])
...
g <- which(mcols(disease.dds)$gene_name == "ITGB1")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "ASNS")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "KIAA1671")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
g <- which(mcols(disease.dds)$gene_name == "TRGV5")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
```{r disease-genes-bmi, warning=FALSE}
g <- which(mcols(disease.dds)$gene_name == "SIRT1")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
