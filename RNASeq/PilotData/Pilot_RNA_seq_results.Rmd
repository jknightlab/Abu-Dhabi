...
```{r data}
counts <- read.delim("PilotRNA-hisat-count-data.txt")
full.s.info <- read.delim("PilotSampleInfoExpanded.txt", stringsAsFactors=F,
                          row.names=1)
...
depth. The "blind"=FALSE argument means that the experimental design is used to estimate global variability in the counts. 
...
lpsresOrdered <- lps.res.LFC[order(lps.res.LFC$pvalue),]

summary(lps.res)
sum(lps.res$padj < 0.05, na.rm=TRUE)
# 15987
...
```{r volcano-plots}
lps.res.LFC$sig <- ifelse(lps.res.LFC$padj < 0.05, "FDR<0.05", "Not Sig")
ggplot(data.frame(lps.res.LFC), aes(log2FoldChange, -log10(padj))) +
