...
genes with lower counts, however, the values are shrunken towards the genes’
...
ggplot(pcaData, aes(x = PC1, y = PC2, color = SampleType, shape = Disease)) +
  ...
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  coord_fixed() +
  # geom_text(aes(y = PC2+1, label=PatientNumber)) +
  theme_bw()

ggplot(pcaData, aes(x=PatientNumber, y=PC1)) +
  geom_boxplot() +
  geom_point() +
  theme_bw()

ggplot(pcaData, aes(x=SampleType, y=PC1)) +
  geom_boxplot() +
  geom_point() +
  theme_bw()
```
...
(controls), taking account of patient (and excluding the 2nd visit samples).
...
lps.res <- results(lps.dds, parallel=TRUE, independentFiltering=TRUE, tidy=TRUE)
...
plotMA(lps.res.LFC, ylim=c(-2.5, 2.5))
...
These results are negatively correlated with the CD14 LPS response from Ben's
healthy volunteer data. And yes, I have checked the direction of the contrasts.
...
DESeq2 won't let me include disease in the model in the normal way. What I could 
do instead is to combine patient and disease as a single factor, but I don't want
to do this as it then doesn't acknowledge the grouping by disease status. Instead
I have analysed the patients and controls separately:
...
# All pilot data
data <- lpsresOrdered$Gene[lpsresOrdered$sig == "Sig DE"]
background <- lpsresOrdered$Gene
