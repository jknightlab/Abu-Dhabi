...
and time point, there are 8 different sample types:
...
ggplot(metrics, aes(Total.Purity.Filtered.Reads.Sequenced, Mapped,
  ...
  ggtitle("Mapped Reads vs Total Reads Sequenced", 
          "This highlights four samples (4 x R115) where a large number of reads were sequenced but not successfully mapped") +
  ylab("Number of reads mapped to human genome") +
...
The mixed up samples are removed as we cannot definitively resolve the issue.

```{r remove}
to.rm <- as.character(metrics$Sample[metrics$Mapping.Rate < 0.4])
mixup <- as.character(full.s.info$SampleId[full.s.info$SampleNameFull %in% 
  c("94_CD14_LPS", "94_CD14_PBS", "148_CD14_LPS", "148_CD14_MET_LPS", 
    "100_CD14_PBS", "100_CD14_LPS", "100_CD14_MET_LPS", "100_CD14_MET",
    "102_CD14_PBS", "102_CD14_LPS", "102_CD14_MET_LPS", "102_CD14_MET")])
to.rm <- c(to.rm, mixup)  
to.rm <- which(full.s.info$SampleId %in% to.rm)
counts.rm <- counts[, -to.rm]
full.s.info <- full.s.info[-to.rm, ]
...
genes <- getBM(attributes=c('ensembl_gene_id', 
                   ...
                   values =rownames(counts(dds)),
                   mart = ensembl)

featureData <- data.frame(Ensembl=rownames(counts(dds)), 
                          Gene=genes$hgnc_symbol[match(rownames(counts(dds)),
                                                       genes$ensembl_gene_id)],
                          GeneType=genes$gene_biotype[match(rownames(counts(dds)),
                                                       genes$ensembl_gene_id)])
...
of change across genes. These are the indiviudals showing discordance in the PCA.
...
for(j in 1:10){
  ...
  if(gene.name == "") gene.name <- as.character(mcols(lps.dds)$Ensembl[i])
...
that this might be highly distorted by one sample in the control group (112):
...
for(i in 1:nrow(lps.dis.res[lps.dis.res$padj < 0.05, ])){
  ...
  if(gene.name == "") gene.name <- as.character(mcols(lps.dds)$Ensembl[which(rownames(lps.dds) == gene)])
...
No genes are differentially expressed between LPS and MET_LPS.
...
g <- which(mcols(disease.dds)$Gene == "TMEM14C")
d <- plotCounts(disease.dds, gene=g, replaced=TRUE,
                intgroup=c("SampleType", "Disease"),
                returnData=TRUE)
ggplot(d, aes(x=Disease, y=count, color=Disease)) + 
...
to.rm <- c("86_CD14_LPS", "115_CD14_MET_LPS", "115_CD14_PBS", "115_CD14_MET",
           "115_CD14_LPS")
