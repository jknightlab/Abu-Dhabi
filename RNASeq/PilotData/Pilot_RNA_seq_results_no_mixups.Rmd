---
title: "Abu Dhabi RNA-seq project: analysis of pilot data set excluding mixups"
...
Samples with poor mapping quality are removed, as well as potential mix-up samples
...
separation between LPS treated and non-LPS treated cells. 
...
the 2nd visit samples. The independent filtering optimisation is visualised below
and the differential expression results summarised:
...
The MA plot shows that differential expression is not overly dependent on mean 
expression levels, and that the fold changes are centered around 0. 
...
lps.con.res.LFC <- lfcShrink(lps.con.dds, coef=10, parallel=TRUE)
...
lps.t2d.res.LFC <- lfcShrink(lps.t2d.dds, coef=8, parallel=TRUE)
...
Boxplots are shown for the two genes that are DE in the Paxgene samples, and the
one gene in the CD14 MET LPS sample. These show that the same trend is shown
across all sample types, but significance is only reached in the Paxgene samples.
...
(Mamane et al 2009). _TMEM14C_ is involved in heme biosynthesis.
